trigger InsertCRFonLosttoFollowup on Lost_to_Follow_Up__c (before insert, before update) {
	
	Set<Id> ids = new Set<Id>();
	for(Lost_to_Follow_Up__c os : Trigger.new){
		...
		for(Lost_to_Follow_Up__c os : Trigger.new){
			for(TrialPatient__c tmpListObj : trialPatientList) {
				if(os.TrialPatient__c == tmpListObj.Id) {
					...
					crfObj.Type__c = 'Lost to Followup Form';
					...
					crfObj.Type__c = 'Lost_to_Follow_Up__c';
