trigger BaselineSymptomsFormBeforeTrigger on BaselineSymptomsForm__c (before insert, before update) {
	
	if(Trigger.isInsert){
		List<BaselineSymptomsForm__c> lstBaselineSymptoms = Trigger.new;
		List<CRF__c> lstCRF = new List<CRF__c>();
		Set<Id> ids = new Set<Id>();
		for(BaselineSymptomsForm__c baselineSymptoms : lstBaselineSymptoms){
			ids.add(baselineSymptoms.TrialPatient__c);
		}
		...
		List<TrialPatient__c> lstTrialPatient = [Select Id, Site__c, t.Patient_Id__c,Trial_Id__c From TrialPatient__c t where Id IN :ids];
		for(BaselineSymptomsForm__c baselineSymptoms : lstBaselineSymptoms){
			for(TrialPatient__c tp : lstTrialPatient) {
				if(tp.Id != baselineSymptoms.TrialPatient__c) continue;
				CRF__c crf = new CRF__c();
				//crf.Name = 'TSD';
				crf.Type__c = 'Type1'; 
				crf.Detail__c = 'Details';
				crf.Status__c = baselineSymptoms.Status__c;
				crf.Patient__c = tp.Patient_Id__c;
				if(!lstTrialPatient.isEmpty()){
					crf.Trial__c = lstTrialPatient[0].Trial_Id__c;
				}
				crf.TrialPatient__c = baselineSymptoms.TrialPatient__c;
				crf.Complete_Date__c = System.today();
				crf.Type__c = 'BaselineSymptomsForm__c';
			...
			for(TrialPatient__c tp : lstTrialPatient) {
				for(BaselineSymptomsForm__c baselineSymptoms : lstBaselineSymptoms){
					if(tp.Patient_Id__c == crf.Patient__c) {
						baselineSymptoms.CRF__c = crf.Id;
					}
				}
			}
		}
	...
	if(Trigger.isUpdate){
		...
		for(BaselineSymptomsForm__c baselineSymptoms : Trigger.new){
			setCrfIds.add(baselineSymptoms.CRF__c);
		}
		List<CRF__c> lstCrf = [select id,Status__c from CRF__c where id IN : setCrfIds];
		for(BaselineSymptomsForm__c baselineSymptoms : Trigger.new){
			for(CRF__c crf : lstCrf){
				if(crf.id == baselineSymptoms.CRF__c && baselineSymptoms.Status__c != Trigger.oldMap.get(baselineSymptoms.id).Status__c){
					crf.Status__c = baselineSymptoms.Status__c;
				}
			}
		}
