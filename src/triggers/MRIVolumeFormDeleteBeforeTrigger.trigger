trigger MRIVolumeFormDeleteBeforeTrigger on MRI_Volume__c (before Delete,after Delete){     
     if(Trigger.isBefore){
        set<Id> setCrfIds = new set <Id>();
        set<Id> setCrfIdsr = new set <Id>();
        List<MRI_Volume__c> lstREF = new List<MRI_Volume__c>();
        List<MRI_Volume__c> lstREFr = new List<MRI_Volume__c>();
        List<MRI_Volume__c> clonedelete= new List<MRI_Volume__c>();
      
        for(MRI_Volume__c rec : trigger.old){
        ...
        lstREFr = [select Id, Status__c,Root_CRF_Id__c,OriginalCRF__c from MRI_Volume__c where OriginalCRF__c in :setCrfIdsr];
        System.debug('===lstREFr==='+ lstREFr);
        
        if(setCrfIdsr.size() > 0){
          for(MRI_Volume__c ae : lstREFr ){
              clonedelete.add(new MRI_Volume__c (id=ae.id));
          }
       ...
       if(setCrfIds.size() > 0){
          for(id ids : setCrfIds){
              ...
              lstREF.add(new MRI_Volume__c (id=ids));
          }
       }
