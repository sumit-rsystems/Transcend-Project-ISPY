trigger InsertCRFOnOffStudy on Off_Study_Detail__c (before insert, before update) {
	
	Set<Id> ids = new Set<Id>();
	for(Off_Study_Detail__c os : Trigger.new){
		...
		for(Off_Study_Detail__c os : Trigger.new){
			for(TrialPatient__c tmpListObj : trialPatientList) {
				if(os.TrialPatient__c == tmpListObj.Id) {
					...
					crfObj.Type__c = 'Off Study Detail';
					...
					crfObj.Type__c = 'Off_Study_Detail__c';
					crfObj.Phase__c = 'Treatment,Followup';
					lstCRF.add(crfObj); 
