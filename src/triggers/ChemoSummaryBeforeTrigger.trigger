trigger ChemoSummaryBeforeTrigger on Chemo_Summary_Form__c (before insert, before update) {
	
	if(Trigger.isInsert){
		List<Chemo_Summary_Form__c> lstCSF = Trigger.new;
		...
		for(Chemo_Summary_Form__c csf : lstCSF){
			for(TrialPatient__c tp : lstTrialPatient) {
				...
				crf.Type__c = 'Chemo_Summary_Form__c';
